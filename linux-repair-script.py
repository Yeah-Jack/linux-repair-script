...
def update_system():
    if ask_yes_no("Do you want to update your system?"):
        ...
        lock_error_pattern = (
            r"Could not get lock .+. It is held by process \d+ \(apt-get\)"
        )
        if re.search(lock_error_pattern, output):
            print_highlight(
                "Error: Another process is using the package management system."
            )
            print_highlight("This could be due to an interrupted update or upgrade.")
            if ask_yes_no(
                "Do you want to run 'sudo dpkg --configure -a' to attempt to fix this?"
            ):
        ...
        if return_code != 0:
            print_highlight(
                "Errors encountered during update. Please check the output above."
            )
        else:
...
def upgrade_fix_missing():
    if ask_yes_no(
        "Do you want to upgrade and fix missing packages? This may take a while."
    ):
...
def upgrade_fix_broken():
    if ask_yes_no(
        "Do you want to upgrade and fix broken packages? This may take a while."
    ):
...
def configure_packages():
    if ask_yes_no(
        "Do you want to configure unconfigured packages? This can be skipped if you ran sudo dpkg --configure -a in the first step. This may take some time."
    ):
...
def clean_system():
    if ask_yes_no(
        "Do you want to clean unnecessary packages and cache? This may take a moment."
    ):
...
def check_disk():
    if ask_yes_no(
        "Do you want to check the disk for errors? This may take a long time."
    ):
...
def repair_filesystem():
    if ask_yes_no(
        "Do you want to check and repair the filesystem? This may take a while."
    ):
...
def update_grub():
    if ask_yes_no(
        "Do you want to update GRUB? This can help if you're having boot issues."
    ):
...
def regenerate_font_cache():
    if ask_yes_no(
        "Do you want to regenerate the font cache? This can help with font-related issues."
    ):
...
def main():
    ...
    print_highlight(
        "This script will help you perform various system maintenance tasks."
    )
    print_highlight(
        "Please note that some tasks may require a significant amount of time."
    )
    print(
        f"{YELLOW}                                                                                                                     _  __     _ _   ____                  _        ____            _       _   "
    )
    print(
        "                                                                                                                    | |/ /__ _| (_) |  _ \ ___ _ __   __ _(_)_ __  / ___|  ___ _ __(_)_ __ | |_ "
    )
    print(
    ...
    print(
        "                                                                                                                    |_|\_\__,_|_|_| |_| \_\___| .__/ \__,_|_|_|    |____/ \___|_|  |_| .__/ \__|"
    )
    print(
        f"                                                                                                                                              |_|                                    |_|        {RESET}"
    )
